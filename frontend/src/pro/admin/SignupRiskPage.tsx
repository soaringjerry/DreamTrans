...
interface Settings { enabled: boolean; device_limit: number; network_daily_limit: number; automatic_daily_limit: number }
interface Profile {
...
export function SignupRiskPage({ run }: { run: Runner }) {
  ...
  async function review(next: 'approved' | 'denied') {
    ...
    setBusy(true)
    const saved = await run(() => adminFetch(`/api/admin/signup-risk/${selected.id}`, { method: 'POST', body: JSON.stringify({ decision: next, note: note.trim() }) }), next === 'approved' ? '已放行；已验证邮箱的账号会领取权益' : '已拒绝自动赠送，账号仍可使用')
    setBusy(false)
    setGeneration((n) => n + 1)
  ...
  return <div className="pa-stack">
    <section className="pa-card pa-promotion-intro">
      ...
      <details><summary>风控规则与阈值</summary>{settings && <form className="pa-dialog-form" onSubmit={(event) => { void save(event) }}>
        ...
        <p className="pa-form-note">浏览器和网络计数包括未验证、已删除账号。关闭规则后只记录信号并自动放行新注册，不会释放已有待审权益。清除 Cookie 可改变浏览器标识，因此它不能单独证明身份。</p>
    ...
    <section className="pa-card pa-promotion-intro">
      <form className="pa-promotion-actions" onSubmit={(event) => { event.preventDefault(); setQuery(search.trim()); setPage(1) }}>
        <select aria-label="审核状态" value={decision} onChange={(event) => { setDecision(event.target.value); setPage(1) }}><option value="">所有状态</option>{Object.entries(labels).map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select>
      ...
      <div className="pa-table-wrap"><table><thead><tr><th>账号 / 渠道</th><th>风险信号</th><th>注册时间 / 状态</th><th>处理</th></tr></thead><tbody>
        {result.profiles.map((p) => <tr key={p.id}>
          ...
          <td>{p.reasons.length ? p.reasons.map((reason) => <div key={reason}>{reasons[reason] || reason}</div>) : '未触发规则'}<small>之前的注册：浏览器 {p.device_count} · 网络 {p.network_count} · 全站放行 {p.daily_count}</small></td>
          <td>{formatDate(p.created_at)}<div>{labels[p.decision]} · {p.verified ? '邮箱已验证' : '邮箱未验证'}</div></td>
    ...
    {selected && <Modal title="注册赠送审核" onClose={() => { if (!busy) setSelected(null) }} footer={<>
      {selected.user_id && selected.decision !== 'allowed' && <button className="pa-button pa-button--primary" disabled={busy || !note.trim()} type="button" onClick={() => { void review('approved') }}>{selected.decision === 'approved' ? '重试权益发放' : '放行赠送权益'}</button>}
