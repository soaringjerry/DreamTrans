...
func ReadSettings(ctx context.Context, q queryRower) (Settings, error) {
	var s Settings
	err := q.QueryRowContext(ctx, `SELECT enabled,device_limit,network_daily_limit,automatic_daily_limit FROM signup_risk_settings WHERE singleton`).Scan(&s.Enabled, &s.DeviceLimit, &s.NetworkDailyLimit, &s.AutomaticDailyLimit)
	return s, err
}
...
func AssessTx(ctx context.Context, tx *sql.Tx, s *Signals) (*Assessment, error) {
	...
	if settings.Enabled && len(a.Reasons) > 0 {
		a.Decision = "review"
	}
	return a, nil
}
func RecordTx(ctx context.Context, tx *sql.Tx, userID string, s *Signals, a *Assessment) error {
	...
	_, err = tx.ExecContext(ctx, `INSERT INTO signup_risk_profiles(user_id,email_hash,device_hash,network_hash,decision,reasons,device_count,network_count,daily_count,rules)
        VALUES($1,$2,NULLIF($3,''),NULLIF($4,''),$5,$6,$7,$8,$9,$10)`, userID, s.EmailHash, s.DeviceHash, s.NetworkHash, a.Decision, reasons, a.DeviceCount, a.NetworkCount, a.DailyCount, rules)
	return err
}
...
func (s *Service) UpdateSettings(ctx context.Context, input Settings, actor string) error {
	...
	if _, err := tx.ExecContext(ctx, `UPDATE signup_risk_settings SET enabled=$1,device_limit=$2,network_daily_limit=$3,automatic_daily_limit=$4,updated_at=NOW(),updated_by=$5 WHERE singleton`, input.Enabled, input.DeviceLimit, input.NetworkDailyLimit, input.AutomaticDailyLimit, actor); err != nil {
		return err
	}
...
func (s *Service) List(ctx context.Context, decision, search string, limit, offset int) ([]Profile, int, error) {
	...
	for rows.Next() {
		var p Profile
		var reasons, rules []byte
		if err := rows.Scan(&p.ID, &p.UserID, &p.Email, &p.Name, &p.Verified, &p.Decision, &reasons, &p.DeviceCount, &p.NetworkCount, &p.DailyCount, &rules, &p.CreatedAt, &p.ReviewedAt, &p.ReviewNote, &p.Promotion, &p.Channel); err != nil {
			return nil, 0, err
		}
...
func validDecisionFilter(value string) bool {
	switch value {
	case "", "allowed", "review", "approved", "denied":
		return true
	}
	return false
}
...
func (s *Service) Review(ctx context.Context, id, decision, note, actor string) (string, error) {
	...
	var previous string
	if err := tx.QueryRowContext(ctx, `SELECT decision FROM signup_risk_profiles WHERE id=$1 FOR UPDATE`, id).Scan(&previous); err != nil {
		return "", err
	}
	if previous == decision {
		return userID, nil
	}
	if previous == "allowed" || previous == "approved" {
		return "", ErrDecision
	}
...
func (s *Service) UserDecision(ctx context.Context, userID string) (string, error) {
	var decision string
	err := s.db.QueryRowContext(ctx, `SELECT decision FROM signup_risk_profiles WHERE user_id=$1`, userID).Scan(&decision)
	if errors.Is(err, sql.ErrNoRows) {
		return "legacy", nil
	}
	return decision, err
...
func (s *Service) PruneSignals(ctx context.Context) error {
	_, err := s.db.ExecContext(ctx, `UPDATE signup_risk_profiles SET device_hash=NULL,network_hash=NULL WHERE created_at<NOW()-INTERVAL '30 days' AND (device_hash IS NOT NULL OR network_hash IS NOT NULL)`)
	if err != nil {
