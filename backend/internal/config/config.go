...
func Load() error {
    ...
    if p == "" {
        p = "/app/data/dreamtrans.config.json"
    }
    path = p
    // Try read
    b, err := os.ReadFile(p)
...
func Update(partial *Config) error {
    ...
    if partial.Prompts.Summary != "" { current.Prompts.Summary = partial.Prompts.Summary }
    // models
    if partial.Models.Translate != "" { current.Models.Translate = partial.Models.Translate }
    if partial.Models.Summary != "" { current.Models.Summary = partial.Models.Summary }
    if partial.Models.Chat != "" { current.Models.Chat = partial.Models.Chat }
    // translation thresholds
    if partial.Translation.MinChunkChars > 0 { current.Translation.MinChunkChars = partial.Translation.MinChunkChars }
    if partial.Translation.FlushGapSeconds > 0 { current.Translation.FlushGapSeconds = partial.Translation.FlushGapSeconds }
    if partial.Translation.KeepLastTranslated > 0 { current.Translation.KeepLastTranslated = partial.Translation.KeepLastTranslated }
    // summary thresholds
    if partial.Summary.MinIntervalSeconds > 0 { current.Summary.MinIntervalSeconds = partial.Summary.MinIntervalSeconds }
    if partial.Summary.MinChars > 0 { current.Summary.MinChars = partial.Summary.MinChars }
