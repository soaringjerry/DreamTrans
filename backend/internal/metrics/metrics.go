// Package metrics records bounded in-process latency and token metrics.
package metrics
...
func ensurePerModel(ft *FeatureTotals, model string) *FeatureTotals {
	if ft.PerModel == nil {
		ft.PerModel = make(map[string]*FeatureTotals)
	}
	m := ft.PerModel[model]
	if m == nil {
		m = &FeatureTotals{}
...
func cloneFeatureTotals(source FeatureTotals) FeatureTotals {
	clone := source
	if source.PerModel == nil {
		return clone
	}
	clone.PerModel = make(map[string]*FeatureTotals, len(source.PerModel))
	for model, totals := range source.PerModel {
		if totals == nil {
			clone.PerModel[model] = nil
			continue
		}
		nested := cloneFeatureTotals(*totals)
		clone.PerModel[model] = &nested
	}
	return clone
}
